-- Restringe a agenda clínica a dias úteis e formaliza a grade de início.
-- Execute depois de 20260806211500_aprimora_fluxo_clinica.sql.
...
drop trigger if exists agendamentos_validar_grade
  on public.agendamentos_ocupacionais;

create trigger agendamentos_validar_grade
before insert or update of inicio_agendado, fim_agendado
on public.agendamentos_ocupacionais
for each row execute function public.validar_grade_de_agendamento();
...
comment on function public.validar_grade_de_agendamento() is
  'Impede início fora da grade de 30 minutos e atendimentos aos sábados ou domingos.';
